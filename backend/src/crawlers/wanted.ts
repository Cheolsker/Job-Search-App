 ...
 */
export async function crawlWantedJobs(keyword: string, limit: number = 20): Promise<Job[]> {
  ...
  try {
    ...
    try {
      ...
      // 페이지가 완전히 로드될 때까지 대기
      // 정확한 선택자 사용
      await page.waitForSelector('.JobList_container__Hf1rb', { timeout: 10000 })
        .catch(() => console.log('JobList container selector not found, but continuing...'));
      ...
      const jobElements = await page.evaluate(() => {
        const elements = Array.from(document.querySelectorAll('.JobCard_container__zQcZs'));
        return elements.map(el => {
          ...
          // 회사명 추출
          const companyElement = el.querySelector('.CompanyNameWithLocationPeriod_CompanyNameWithLocationPeriod__company__ByVLu');
          const company = companyElement ? companyElement.textContent?.trim() || '' : '';
          
          // 경력 추출
          const locationElement = el.querySelector('.CompanyNameWithLocationPeriod_CompanyNameWithLocationPeriod__location__4_w0l');
          const experience = locationElement ? locationElement.textContent?.trim() || '' : '';
          
          // 계약직 여부 확인
          const contractElement = el.querySelector('.wds-5jjoh5');
          const contractType = contractElement ? contractElement.textContent?.trim() || '' : '';
          
          // 합격 보상금 추출
          const rewardElement = el.querySelector('.JobCard_reward__oCSIQ');
          const reward = rewardElement ? rewardElement.textContent?.trim() || '' : '';
          ...
          const jobCategory = linkElement ? linkElement.getAttribute('data-job-category') || '개발' : '개발';
